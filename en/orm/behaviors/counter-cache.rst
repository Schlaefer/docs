...
``$original`` is ``true``. If the update changes the association it is called
a *second* time with ``false``, the return value then updates the cache counter
of the *previously* associated item.
